...
function renderConfig() {
  ...
  el.innerHTML = html + panelEditorHtml();
}

/* =========================================================
   PANEL — mostrar/ocultar, renombrar y reordenar las tarjetas
   de la pestaña Resumen (en caja.html).
========================================================= */
function panelEditorHtml() {
  ...
  function filaEditor(tabId, def) {
    const resuelto = cajaPanelResolver(tabId, [def])[0];
    return '<div class="caja-row" style="align-items:center; margin-bottom:8px;">' +
      '<input type="checkbox" ' + (resuelto.visible ? "checked" : "") + ' onchange="cajaPanelGuardar(\'' + tabId + '\', \'' + def.id + '\', \'visible\', this.checked)" style="width:auto;">' +
      '<input class="caja-input" style="flex:2;" value="' + escapeHtml(resuelto.titulo) + '" onchange="cajaPanelGuardar(\'' + tabId + '\', \'' + def.id + '\', \'titulo\', this.value)">' +
      '<input class="caja-input" type="number" style="flex:0 0 60px;" value="' + resuelto.orden + '" title="Orden" onchange="cajaPanelGuardar(\'' + tabId + '\', \'' + def.id + '\', \'orden\', parseFloat(this.value)||0)">' +
      "</div>";
  }

  let html = '<div class="caja-card">';
  html += '<div class="stat-label" style="margin-bottom:6px;">Panel — pestaña Resumen (en Caja)</div>';
  html += '<div class="stat-sub" style="margin-bottom:12px;">Tildado = visible. El número de la derecha define el orden (más chico aparece primero). Cambiá el texto para renombrar cada tarjeta.</div>';

  html += '<div style="font-size:0.75rem; color:var(--accent); font-weight:700; margin:10px 0 6px;">Secciones principales</div>';
  bloquesDef.forEach(function (d) { html += filaEditor("resumen", d); });

  html += '<div style="font-size:0.75rem; color:var(--accent); font-weight:700; margin:14px 0 6px;">Caja en efectivo — tarjetas</div>';
  cajaDefs.forEach(function (d) { html += filaEditor("resumen-caja", d); });

  html += '<div style="font-size:0.75rem; color:var(--accent); font-weight:700; margin:14px 0 6px;">Ventas — tarjetas</div>';
  ventasDefs.forEach(function (d) { html += filaEditor("resumen-ventas", d); });

  html += '<button class="caja-btn caja-btn-ghost" style="margin-top:10px;" onclick="restablecerPanel()">Restablecer panel de Resumen</button>';
  html += '<div class="stat-sub" style="margin-top:8px;">Los cambios se ven al abrir o recargar Caja → Resumen.</div>';
  html += '</div>';

  return html;
}

function restablecerPanel() {
  if (!confirm("¿Restablecer el panel de Resumen a los valores originales?")) return;
  cajaPanelRestablecer("resumen");
  cajaPanelRestablecer("resumen-caja");
  cajaPanelRestablecer("resumen-ventas");
  renderConfig();
}
