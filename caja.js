...
function movRowHtml(m) {
  ...
  return (
    '<div class="mov-row" onclick="abrirDetalleMovimiento(\'' + m.id + '\')">' +
    ...
    '<button class="mov-del" onclick="event.stopPropagation(); borrarMovimiento(\'' + m.id + '\')"><i class="fa-solid fa-trash"></i></button>' +
    "</div>"
  );
}

/* =========================================================
   DETALLE DE MOVIMIENTO / VENTA (modal)
========================================================= */
function abrirDetalleMovimiento(id) {
  ...
  const isIn = m.type === "ingreso";
  const overlay = document.getElementById("mov-detail-modal");
  const box = document.getElementById("mov-detail-content");
  if (!overlay || !box) return;

  let html = '<div class="mc-modal-title"><h3>' + (isIn ? "Ingreso" : "Egreso") + '</h3><button class="mc-modal-close" onclick="cerrarDetalleMovimiento()"><i class="fa-solid fa-xmark"></i></button></div>';

  html += '<div style="text-align:center; margin-bottom:10px;">' +
    '<div style="font-family:\'Courier New\',monospace; font-size:1.6rem; font-weight:800; color:' + (isIn ? "#5fa372" : "#e2564a") + ';">' +
    (isIn ? "+" : "\u2212") + cajaFmtMoney(m.amount) + '</div></div>';

  html += '<div class="mc-detail-row"><span>Fecha</span><span>' + cajaFmtDateLabel(m.date) + " (" + m.date + ")</span></div>";
  html += '<div class="mc-detail-row"><span>Categoría</span><span>' + escapeHtml(m.category) + "</span></div>";
  html += '<div class="mc-detail-row"><span>Método de pago</span><span>' + escapeHtml(m.method) + "</span></div>";
  html += '<div class="mc-detail-row"><span>Caja</span><span>' + (m.ledger === "principal" ? "Caja mayor" : "Caja chica") + "</span></div>";
  if (m.note) html += '<div class="mc-detail-row"><span>Nota</span><span>' + escapeHtml(m.note) + "</span></div>";

  if (m.items && m.items.length) {
    html += '<div class="stat-label" style="margin:14px 0 8px;">Productos de esta venta</div>';
  ...
  html += '<button class="caja-btn caja-btn-ghost caja-btn-block" style="margin-top:16px;" onclick="borrarMovimiento(\'' + m.id + '\'); cerrarDetalleMovimiento();"><i class="fa-solid fa-trash"></i> Borrar este movimiento</button>';
...
function renderResumen() {
  ...
  const ingresoEfectivoHoy = todayTx.filter(function (t) { return t.type === "ingreso" && t.method === "Efectivo"; }).reduce(function (a, t) { return a + t.amount; }, 0);
  const egresoEfectivoHoy = todayTx.filter(function (t) { return t.type === "egreso" && t.method === "Efectivo"; }).reduce(function (a, t) { return a + t.amount; }, 0);
  const otrosMediosHoy = todayTx.filter(function (t) { return t.type === "ingreso" && t.method !== "Efectivo"; }).reduce(function (a, t) { return a + t.amount; }, 0);

  const ventasHoy = todayTx.filter(function (t) { return t.type === "ingreso"; });
  const totalDia = ventasHoy.reduce(function (a, t) { return a + t.amount; }, 0);
  const totalMes = monthTx.filter(function (t) { return t.type === "ingreso"; }).reduce(function (a, t) { return a + t.amount; }, 0);
  const ticketsHoy = ventasHoy.reduce(function (a, t) { return a + (t.tickets || 1); }, 0);

  const porMetodoHoy = {};
  ventasHoy.forEach(function (t) { porMetodoHoy[t.method] = (porMetodoHoy[t.method] || 0) + t.amount; });

  const recent = state.transactions.slice().sort(function (a, b) { return a.date < b.date ? 1 : -1; }).slice(0, 6);

  // --- Bloque: Caja en efectivo ---
  const cajaDefs = [
    { id: "ingreso-efectivo", titulo: "Ingreso efectivo (hoy)", orden: 1 },
    { id: "egreso-efectivo", titulo: "Egreso efectivo (hoy)", orden: 2 },
    { id: "otros-medios", titulo: "Ingresos por otros medios (hoy)", orden: 3 },
    { id: "total-caja", titulo: "Total caja final (efectivo)", orden: 4 },
  ];
  const cajaValores = {
  ...
  const cajaHtml = cajaResueltos.length === 0 ? "" :
    '<div class="stat-grid">' +
    cajaResueltos.map(function (s) {
      const v = cajaValores[s.id];
      const destacar = s.id === "total-caja";
      return '<div class="stat-card"' + (destacar ? ' style="border-color:var(--accent);"' : "") + '><div class="stat-label">' + escapeHtml(s.titulo) + '</div><div class="stat-value ' + v.tono + '"' + (destacar ? ' style="color:var(--accent); font-size:1.4rem;"' : "") + '>' + v.valor + "</div></div>";
    }).join("") +
    "</div>";

  // --- Bloque: Ventas ---
  const ventasDefs = [
    { id: "por-metodo", titulo: "Ventas por método de pago (hoy)", orden: 1 },
    { id: "tickets", titulo: "Tickets hoy", orden: 2 },
    { id: "total-dia", titulo: "Total del día", orden: 3 },
    { id: "total-mes", titulo: "Total del mes", orden: 4 },
  ];
  const ventasResueltos = cajaPanelResolver("resumen-ventas", ventasDefs).filter(function (s) { return s.visible; });

  function ventaBloqueHtml(id, titulo) {
    if (id === "por-metodo") {
      return '<div class="caja-card">' +
        '<div class="stat-label" style="margin-bottom:8px;">' + escapeHtml(titulo) + '</div>' +
        (Object.keys(porMetodoHoy).length === 0
          ? '<div class="empty-note">Sin ventas hoy todavía.</div>'
          : Object.keys(porMetodoHoy).map(function (m) {
              return '<div class="report-cat-row"><span>' + escapeHtml(m) + '</span><span>' + cajaFmtMoney(porMetodoHoy[m]) + '</span></div>';
            }).join("")) +
        "</div>";
    }
    const valores = { tickets: ticketsHoy, "total-dia": cajaFmtMoney(totalDia), "total-mes": cajaFmtMoney(totalMes) };
    return '<div class="stat-card"><div class="stat-label">' + escapeHtml(titulo) + '</div><div class="stat-value up">' + valores[id] + "</div></div>";
  }

  const ventasStatsHtml = ventasResueltos.filter(function (v) { return v.id !== "por-metodo"; })
    .map(function (v) { return ventaBloqueHtml(v.id, v.titulo); }).join("");
  const ventasMetodoItem = ventasResueltos.find(function (v) { return v.id === "por-metodo"; });

  const ventasHtml =
    (ventasStatsHtml ? '<div class="stat-grid">' + ventasStatsHtml + "</div>" : "") +
    (ventasMetodoItem ? ventaBloqueHtml("por-metodo", ventasMetodoItem.titulo) : "");

  // --- Bloques principales de la pestaña ---
  const bloquesDef = [
    { id: "caja-efectivo", titulo: "Caja en efectivo", orden: 1 },
    { id: "ventas", titulo: "Ventas", orden: 2 },
    { id: "ultimos-movimientos", titulo: "Últimos movimientos", orden: 3 },
  ];
  const bloquesResueltos = cajaPanelResolver("resumen", bloquesDef);
  function tituloDe(id) { return bloquesResueltos.find(function (b) { return b.id === id; }).titulo; }

  const bloqueHtml = {
    "caja-efectivo": '<div class="caja-section-title"><h2>' + escapeHtml(tituloDe("caja-efectivo")) + '</h2></div>' + cajaHtml,
    "ventas": '<div class="caja-section-title"><h2>' + escapeHtml(tituloDe("ventas")) + '</h2></div>' + ventasHtml,
    "ultimos-movimientos": '<div class="caja-section-title"><h2>' + escapeHtml(tituloDe("ultimos-movimientos")) + '</h2></div>' +
      '<div class="caja-card">' +
      (recent.length === 0 ? '<div class="empty-note">Todavía no hay movimientos.</div>' : recent.map(movRowHtml).join("")) +
      "</div>",
  };

  el.innerHTML = bloquesResueltos.filter(function (b) { return b.visible; }).map(function (b) { return bloqueHtml[b.id]; }).join("");
}
...
function renderReportes() {
  ...
  const ranking = cajaProductRanking(state.transactions, { desde: thisMonth + "-01", hasta: thisMonth + "-31" }).slice(0, 10);
  const maxRankQty = Math.max(1, Math.max.apply(null, ranking.map(function (r) { return r.cantidad; }).concat([0])));
  ...
  el.innerHTML =
    ...
    '<div class="caja-card">' +
    '<div class="stat-label" style="margin-bottom:10px;">Ranking de productos vendidos &mdash; mes actual</div>' +
    (ranking.length === 0 ? '<div class="empty-note">Todavía no hay ventas con detalle de productos este mes (solo cuentan las hechas desde Pedidos).</div>' :
      ranking.map(function (r, i) {
        return '<div style="margin-bottom:8px;">' +
          '<div style="display:flex; justify-content:space-between; font-size:0.78rem; margin-bottom:3px;">' +
          '<span style="color:var(--text-white);">' + (i + 1) + '. ' + escapeHtml(r.nombre) + '</span>' +
          '<span style="font-family:\'Courier New\',monospace; color:var(--text-gray);">' + r.cantidad + ' un. &middot; ' + cajaFmtMoney(r.total) + '</span></div>' +
          '<div class="progress-track"><div class="progress-fill" style="width:' + (r.cantidad / maxRankQty * 100) + '%"></div></div></div>';
      }).join("")) +
    "</div>" +
